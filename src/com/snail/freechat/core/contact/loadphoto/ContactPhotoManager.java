package com.snail.freechat.core.contact.loadphoto;
...
import com.snail.freechat.util.UriUtils;
import com.snail.freechat.util.image.BitmapUtil;
import com.snail.freechat.util.memory.MemoryUtils;
...
public abstract class ContactPhotoManager implements ComponentCallbacks2 {
    ...
    public static void setDefaultAvatar(int resId) {
        defaultAvatar = resId;
    }

    public static int getDefaultAvatarResId(boolean hires, boolean darkTheme) {
        if (hires && darkTheme) return defaultAvatar;
        if (hires) return defaultAvatar;
        if (darkTheme) return defaultAvatar;
    ...
    private static class BlankDefaultImageProvider extends DefaultImageProvider {
        ...
        public void applyDefaultImage(ImageView view, int extent, boolean darkTheme) {
            if (sDrawable == null) {
                ...
                sDrawable = new ColorDrawable(Color.WHITE);
            }
    ...
    /**
     * Requests the singleton instance of {@link } with data bound from
    ...
    /**
     * Calls {@link #(android.widget.ImageView, android.net.Uri, boolean, boolean, DefaultImageProvider)} with
     * {@link #DEFAULT_AVATAR}.
     */
...
class ContactPhotoManagerImpl extends ContactPhotoManager implements Handler.Callback {
    ...
    private static final String[] COLUMNS = new String[]{ContactsContract.Contacts.Photo._ID, ContactsContract.Contacts.Photo.PHOTO};
...
//		}
    ...
    private class LoaderThread extends HandlerThread implements Handler.Callback {
        ...
        private void queryPhotosForPreload() {
            Cursor cursor = null;
            try {
                Uri uri = ContactsContract.Contacts.CONTENT_URI.buildUpon().appendQueryParameter(
                        ...
                        .build();
                cursor = mResolver.query(uri, new String[]{ContactsContract.Contacts.PHOTO_ID},
                        ContactsContract.Contacts.PHOTO_ID + " NOT NULL AND " + ContactsContract.Contacts.PHOTO_ID + "!=0",
                        null,
                        ContactsContract.Contacts.STARRED + " DESC, " + ContactsContract.Contacts.LAST_TIME_CONTACTED + " DESC");
        ...
        private void loadThumbnails(boolean preloading) {
            ...
            mStringBuilder.append(ContactsContract.Contacts.Photo._ID + " IN(");
            ...
            try {
                ...
                cursor = mResolver.query(ContactsContract.Data.CONTENT_URI,
                        COLUMNS,
